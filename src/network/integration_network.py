...
#data_path = "/Users/tatiana/Documents/Magistracy/MBA"

dataset_path = "E:\Projects\MBA_retail\\tmp\datasets" #for change_train
#dataset_path = "/Users/tatiana/Documents/Magistracy/MBA"
...
m = int(dataset[len(dataset)-1][0])
dict_orders_orig = {}
counter_nodes = 0
degrees = []
inf = (nx.info(network))
degrees.append(float(inf[-7:]))
print(float(inf[-7:]))
for j in range(number_of_ex_orders):
    print(j+1, '/', number_of_ex_orders)
    counter_edges = 0
    network.add_node(m+external_dataset[j][0])
    for i in range(number_of_orders):
        weight_1 = 0
        weight_1 = sum(dataset[i] & external_dataset[j])

        if weight_1 != 0 and network.has_node(str(dataset[i][0])):
            weight = weight_1
            network.add_edge(str(dataset[i][0]), m+external_dataset[j][0], weight=weight)
        if j == 0:
    ...
    if(abs(degrees[-2] - degrees[-1]) > 1.9): #1.6
        network.remove_node(m + external_dataset[j][0])
        del degrees[-1]
    else:
        dict_orders_orig.update({m + external_dataset[j][0]: j})
        counter_nodes += 1
...
for i in ns:
    ...
    if int(i) > m:
        integrated_dataset.append(external_dataset[i2])
        cleaned_groc.append(external_dataset[i2])

my_df = pd.DataFrame(integrated_dataset)
my_df.to_csv('{0}/integrated_train_network_v2.csv'.format(data_path), index=False, header=False)


my_df2 = pd.DataFrame(cleaned_groc)
my_df2.to_csv('{0}/cleaned_groc_network_v2.csv'.format(data_path), index=False, header=False)